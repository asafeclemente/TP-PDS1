...
// first let's count how many lines the llvm code has to allocate the right number of pointers to llvm.lines
	...
	int line = 0; //position in the vetor size_of_lines	
...
LLVM get_common_subsequence(int** matrix, LLVM llvm_1, LLVM llvm_2)
{
	...
	common_code.number_of_lines =  matrix[i][j];
	int index = common_code.number_of_lines;  //index is the number of lines of common code

	common_code.lines = (char**) malloc(index * sizeof(char*));

	while (i > 0 && j > 0 && index >= 0){ 
		if (strcmp(llvm_1.lines[i-1], llvm_2.lines[j-1]) == 0){ //in this case the lines are equal
			common_code.lines[index-1] = (char*) malloc(strlen(llvm_1.lines[i-1]) + 1 * sizeof(char));
			strcpy(common_code.lines[index-1], llvm_1.lines[i-1]);
			index--; 
			j--;
			i--;
		}
...
void free_LLVM(LLVM llvm){
	for (int i=0; i < llvm.number_of_lines; i++){
		free (llvm.lines[i]);
		llvm.lines[i] = NULL;
	}
	free (llvm.lines);
